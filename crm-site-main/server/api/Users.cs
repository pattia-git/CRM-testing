...
async Task<IResult> CreateEmployee(HttpContext context, CreateEmployeeRequest createEmployeeRequest)
{
    try
    {
        ...
        await using var cmd = Db.CreateCommand("SELECT id FROM companies WHERE name = @company");
        ...
        if (companyId != null)
        {
            await using var cmd2 = Db.CreateCommand(@"
                INSERT INTO users 
                (firstname, lastname, username, password, role, email, company)
                VALUES 
                (@firstname, @lastname, @username, @password, @role::role, @email, @company_id)
                RETURNING id;
            ");
            ...
            try
            {
                var newUserId = await cmd2.ExecuteScalarAsync();
                return Results.Ok(new { message = "User registered.", userId = newUserId });
            }
            catch (Exception ex)
            {
